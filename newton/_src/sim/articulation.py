...
@wp.func
def body_com_world_offset(X_wb: wp.transform, body_com: wp.vec3):
    """Return the world-space offset from body origin to COM."""
    return wp.transform_vector(X_wb, body_com)


@wp.func
def body_com_world_position(X_wb: wp.transform, body_com: wp.vec3):
    """Return the body COM position in world coordinates."""
    return wp.transform_point(X_wb, body_com)
...
def com_twist_to_point_velocity(qd: wp.spatial_vector, X_wb: wp.transform, body_com: wp.vec3, point: wp.vec3):
    ...
    return velocity_at_point(qd, point - body_com_world_position(X_wb, body_com))
...
def origin_twist_to_com_twist(qd: wp.spatial_vector, X_wb: wp.transform, body_com: wp.vec3):
    ...
    v_com = velocity_at_point(qd, body_com_world_offset(X_wb, body_com))
...
def com_twist_to_origin_twist(qd: wp.spatial_vector, X_wb: wp.transform, body_com: wp.vec3):
    ...
    omega = wp.spatial_bottom(qd)
    v_origin = wp.spatial_top(qd) - wp.cross(omega, body_com_world_offset(X_wb, body_com))
...
def eval_articulation_ik(
    ...
    if type == JointType.FREE or type == JointType.DISTANCE:
        ...
        x_child_com_world = body_com_world_position(X_wc, body_com[child])
...
def eval_articulation_jacobian(
    ...
    for i in range(joint_count):
        ...
        if type == JointType.FREE or type == JointType.DISTANCE:
            x_joint_com_world = body_com_world_position(body_q[joint_child[j]], body_com[joint_child[j]])
            axis_world_x = wp.transform_vector(X_wpj, wp.vec3(1.0, 0.0, 0.0))
            axis_world_y = wp.transform_vector(X_wpj, wp.vec3(0.0, 1.0, 0.0))
            axis_world_z = wp.transform_vector(X_wpj, wp.vec3(0.0, 0.0, 1.0))
            ...
            joint_S_s[qd_start + 3] = wp.spatial_vector(-wp.cross(axis_world_x, x_joint_com_world), axis_world_x)
            joint_S_s[qd_start + 4] = wp.spatial_vector(-wp.cross(axis_world_y, x_joint_com_world), axis_world_y)
            joint_S_s[qd_start + 5] = wp.spatial_vector(-wp.cross(axis_world_z, x_joint_com_world), axis_world_z)
        else:
    ...
    for i in range(joint_count):
        ...
        j = joint_start + i
        child = joint_child[j]
        x_com_world = body_com_world_position(body_q[child], body_com[child])
        while j != -1:
